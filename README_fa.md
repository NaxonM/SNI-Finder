...
### ۲. تنظیم دامنه‌ها
لیست دامنه‌ها را در `config/sni-list.txt` (یک دامنه در هر خط) وارد کنید. پیشنهاد می‌کنیم **[راهنمای جامع پیدا کردن SNIهای معتبر کلودفلر](docs/sourcing_snis_fa.md)** را مطالعه کنید (شامل فیلترکننده خودکار Tranco).
...
* **نصب خودکار و دور زدن سریع**: ابتدا نصب بودن کتابخانه‌های مورد نیاز (`requests`، `socks`، `rich`) را بررسی می‌کند. اگر نصب نباشند، آن‌ها را از `requirements.txt` نصب می‌کند. در دفعات بعدی، این مرحله به سرعت (در کمتر از یک ثانیه) دور زده می‌شود تا برنامه بدون تاخیر اجرا شود.
...
* **ایزوله‌سازی مخزن و مدیریت وابستگی‌ها**: اسکریپت `start.sh` وجود **محیط مجازی پایتون (`venv`)** را بررسی کرده، در صورت نیاز آن را ساخته و فعال می‌کند. پیش‌نیازها به صورت خودکار در اولین اجرا نصب شده و در اجراهای بعدی بدون اتلاف وقت عبور داده می‌شوند.
